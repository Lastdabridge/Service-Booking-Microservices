...
// Config хранит все настройки сервиса, прочитанные из .env файла.
type Config struct {
	HTTPPort    string
	DatabaseURL string
	KafkaBroker string
	...
	// Топики в которые ПИШЕМ
	TopicAuditEvents         string
	TopicNotificationsOut    string
}

// Load читает .env файл и возвращает Config.
// Если .env нет — читает переменные окружения напрямую (так работает в Docker).
func Load() *Config {
